...
// 以下是实现的系统调用符号常数，用作系统调用函数表中索引值(参见include/linux/sys.h) 。
 #define __NR_setup      0       /* __NR_setup仅用于初始化，以启动系统 */
 #define __NR_exit       1
 #define __NR_fork       2
 #define __NR_read       3
 #define __NR_write      4
...
#define __NR_sigreturn 87

typedef int (*_syscall0_fun)(int callNum);
typedef int (*_syscall1_fun)(int callNum, void *arg1);
typedef int (*_syscall2_fun)(int callNum, void *arg1,void *arg2);
typedef int (*_syscall3_fun)(int callNum, void *arg1,void *arg2,void*arg3);
extern int _syscall(int callNum,void* arg1,void*arg2,void*arg3);
...
{                            \
    int __res;\
    _syscall0_fun cfun;       \
    cfun=(_syscall0_fun)_syscall; \
    __res=cfun(__NR_##name);\
    if (__res >= 0) \
        return (type) __res; \
    errno = -__res; \
    return -1; \
...
{                            \
    int __res;\
    _syscall1_fun cfun;       \
    cfun=(_syscall1_fun)_syscall; \
    __res=cfun(__NR_##name,(void*)a);\
    if (__res >= 0) \
        return (type) __res; \
    errno = -__res; \
    return -1; \
...
{                            \
    int __res;\
    _syscall2_fun cfun;       \
    cfun=(_syscall2_fun)_syscall; \
    __res=cfun(__NR_##name,(void*)a,(void*)b);\
    if (__res >= 0) \
        return (type) __res; \
    errno = -__res; \
    return -1; \
...
{                            \
    int __res;\
    _syscall3_fun cfun;       \
    cfun=(_syscall3_fun)_syscall; \
    __res=cfun(__NR_##name,(void*)a,(void*)b,(void*)c);\
    if (__res >= 0) \
        return (type) __res; \
    errno = -__res; \
    return -1; \
...
// int mknod(const char * filename, mode_t mode, dev_t dev.c);
...
// int ustat(dev_t dev.c, struct ustat * ubuf);
