...
u8 __attribute__((aligned(4))) SDIO_DATA_BUFFER[512];
...
//nblks:要读取的块数
//返回值:错误状态
u32 *  __attribute__((aligned(4)))tempbuff;
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	if (DeviceMode == SD_POLLING_MODE)
	{
		...
		while(!(SDIO->STA&((1<<10)|(1<<4)|(1<<1)|(1<<3)|(1<<9))))//数据块发送成功/下溢/CRC/超时/起始位错误
		{
			...
			{
				if(timeout==0){RestoreCpuInter(t);
					return SD_DATA_TIMEOUT;
				}
				timeout--;
			}
		...
		{RestoreCpuInter(t);
			...
			{															 
				...
				{   
					...
					errorstatus=CmdResp1Error(SD_CMD_STOP_TRANSMISSION);//等待R1响应   
					if(errorstatus!=SD_OK){RestoreCpuInter(t);
						return errorstatus;
					}						
				}
			}
...
u8 SD_Write_FF(u32 sector,u8 cnt){
	u8 sta=SD_OK;
	u8 n;
	long long lsector=sector;
	if(CardType!=SDIO_STD_CAPACITY_SD_CARD_V1_1)lsector<<=9;

	memset(SDIO_DATA_BUFFER,0xff,sizeof(SDIO_DATA_BUFFER));
	if(cnt==1)sta=SD_WriteBlock(SDIO_DATA_BUFFER,lsector,512);    	//单个sector的写操作
	else sta=SD_WriteMultiBlocks(SDIO_DATA_BUFFER,lsector,512,cnt);	//多个sector  
