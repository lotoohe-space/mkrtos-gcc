...
extern volatile PSemHandler dm9000input;				//DM9000接收数据信号量
extern volatile PMutexHandler dm9000lock;				//DM9000读写互锁控制信号量
...
u8 DM9000_Init(u8 mode)
{
	...
	//注册中断
	RegIsrFunc(EXTI15_10_IRQHandler,40,0);
...
//	u8 err;	
 	//printf("send len:%d\r\n",p->tot_len);
	MutexLockISR(dm9000lock,0xffffffff); 			//请求互斥信号量,锁定DM9000 
	...
	MutexUnlockISR(dm9000lock);								//发送互斥信号量,解锁DM9000
}
//DM9000接收数据包
...
void DMA9000_ISRHandler(void)
{
	...
	{  
 		SemPostISR(dm9000input);		//处理接收到数据帧 
	} 
