...
top:	sigchk();
	...
	case nVarsub: case nWord:
		exec(rc_glob(glom(n)), parent);	/* simple command */
		break;
	case nBody:
		walk(n->u[0].p, TRUE);
		WALK(n->u[1].p, parent);
	...
	case nForin: {
		...
		for (l = listcpy(rc_glob(glom(n->u[1].p)), nalloc); l != NULL; l = l->n) {
			Edata block;
	...
	case nAssign:
		...
		assign(glom(n->u[0].p), rc_glob(glom(n->u[1].p)), FALSE);
		set(TRUE);
		break;
	case nPipe:
		dopipe(n);
	...
	case nDup:
		...
		break; /* Null command */
	case nMatch: {
		List *a = rc_glob(glom(n->u[0].p)), *b = glom(n->u[1].p);
		if (dashex)
	...
	case nPre: {
		...
		if (n->u[0].p->type == nRedir || n->u[0].p->type == nDup) {
			...
			if (isallpre(n->u[1].p)) {
				...
				assign(v, rc_glob(glom(n->u[0].p->u[1].p)), TRUE);
				var.name = v->w;
